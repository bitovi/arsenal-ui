...
var GlobalParameterBar = Component.extend({
  ...
  init: function() {
    ...
    Promise.all([
      ...
      self.scope.licensors.replace(values[3]["entities"][0]["entities"]);
      self.scope.contenttypes.replace(values[4]["contentTypes"]);
      //self.scope.periodFrom.replace(values[3]);
      //self.scope.periodTo.replace(values[6]);
      //console.log("Licensors are "+JSON.stringify(values[3]["entities"][0]["entities"].attr()));

    ...
    var self = this;
     ...
     onAccountBalance.findAll().then(function(rows) {
       var footerRows = getFooterRows(quarters,rows);
        ...
        //   //$('#claimLicencorGrid').html(stache('<on-account-grid emptyrows="{emptyrows}"></on-account-grid>')({emptyrows:true}));
        //   $('#onAccount').html(stache('<on-account-grid  rows="{rows}" footerrows="{footerrows}" ></on-account-grid >')({rows, footerrows}));
  ...
  events: {
    'inserted': function(ev) {
      ...
      // Grid.prototype['inserted'] && Grid.prototype['inserted'].apply(this, arguments);
        ...
        //     "ContentType": "Music",
        //     "Q1FY13":"120",
        //     "onAccountBalance":"420",
        //     "cashAdjust":"150"
        //   };
       
        //   var footerrows = [];
        //   footerrows.push(data);

        //   self.scope.footerrows.replace(footerrows);

          //$('#onAccount').html(stache('<on-account-grid footerrows="{footerrows}" emptyrows="{emptyrows}"></on-account-grid>')({footerrows, emptyrows:false}));
...
var frameFooter=function(currencyPeriodArray,totalMap,quarters){
  ...
  for(var k=0; k<currencies.length;k++){
    ...
    childRow["ContentType"]= "";
      for(var i=0;i<quarters.length;i++){
        childRow[quarters[i]]=currencyPeriodArray[currencies[k]];
      }
